...
const mocks = vi.hoisted(() => ({
  ...
  fetchQuery: vi.fn(), clearDrafts: vi.fn(), clearImportRecovery: vi.fn(), clearLinkHealth: vi.fn(), clearCloudUsage: vi.fn(), updateAccountProfile: vi.fn(), invalidateQueries: vi.fn(),
}));
...
vi.mock("../lib/cloudBookmarkUsageQueue", () => ({ clearCloudBookmarkUsage: mocks.clearCloudUsage, flushCloudBookmarkUsage: vi.fn().mockResolvedValue(undefined) }));
...
describe("local routing through cloud failure", () => {
  ...
  beforeEach(() => {
    ...
    mocks.clearCloudUsage.mockReset().mockResolvedValue(undefined);
